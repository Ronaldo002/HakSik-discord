...
from keep_alive import keep_alive  # 24시간 유지를 위한 보조 파일 불러오기
...
async def get_menu(ctx):
    loading_msg = await ctx.send("🔍 금정회관 교직원 식당 메뉴를 조회 중입니다...")
    ...
    try:
        ...
        if table:
            # [수직 낙하 알고리즘] 오늘 요일이 몇 번째 기둥인지 확인
            today_col_index = -1
            thead = table.find("thead")
            headers = thead.find_all(["th", "td"]) if thead else table.find("tr").find_all(["th", "td"])
                
            for i, header in enumerate(headers):
                if today_kr in header.get_text():
                    today_col_index = i
                    break

            if today_col_index != -1:
                tbody = table.find("tbody") if table.find("tbody") else table
                for tr in tbody.find_all('tr'):
                    cells = tr.find_all(['th', 'td'])
                    
                    # 셀 병합(rowspan)으로 인한 칸 밀림 보정
                    actual_index = today_col_index
                    if len(cells) < len(headers):
                        diff = len(headers) - len(cells)
                        actual_index = today_col_index - diff
                    
                    if 0 <= actual_index < len(cells):
                        today_cell = cells[actual_index]
                        for br in today_cell.find_all("br"):
                            br.replace_with("\n")
                        menu_text = today_cell.get_text(separator="\n", strip=True)
                        
                        if menu_text:
                            # 메뉴 구분(중식 등) 추출
                            meal_type = "메뉴"
                            if cells[0] != today_cell: 
        ...
        if not target_menu.strip():
            target_menu = "오늘은 메뉴가 등록되지 않았거나 휴무입니다."
        ...
        await loading_msg.edit(content=f"🍱 **[오늘의 학식] 금정회관 교직원 식당**\n📅 **{date_str} ({today_kr})**\n{formatted_menu}")

    except Exception as e:
        await loading_msg.edit(content=f"❌ 데이터 조회 중 오류가 발생했습니다: {e}")

# 2. 24시간 유지 기능 실행 (웹 서버 가동)
keep_alive()

# -------------------------------------------------------------------
# ⚠️ 깃허브 업로드용 보안 코드 (환경 변수에서 토큰을 불러옵니다)
# -------------------------------------------------------------------
